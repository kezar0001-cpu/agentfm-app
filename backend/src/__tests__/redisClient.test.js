...
describe('redisClient connection handling', () => {
  ...
  it('returns a connected client from getConnectedRedisClient', async () => {
    const fakeClient = {
      isOpen: false,
      connectCalls: 0,
      on() {
    ...
    const connected = await redisModule.getConnectedRedisClient();

    assert.equal(fakeClient.connectCalls, 1);
    assert.equal(connected.isOpen, true);
  });
  ...
  it('falls back to the noop client when connection fails', async () => {
    ...
    const connectedClient = await redisModule.getConnectedRedisClient();

    assert.equal(fakeClient.connectCalls, 1);
    assert.equal(fakeClient.setCalls, 0);
    assert.equal(result, null);
    assert.equal(connectedClient.isOpen, false);
  });
});
